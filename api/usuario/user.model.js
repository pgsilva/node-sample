...
findByLogin = (login) => {
    return new Promise((resolve, reject) => {
        const query = "select * from login where dsemail like concat('%', ? , '%') or dsuser like concat('%', ? , '%')";
...
module.exports = {
    ...
    register: (user) => {
        return new Promise(async (resolve, reject) => {
            ...
            const queryUsuario = 'INSERT INTO usuario(nmusuario, dsemail, nrcelular, idlogin) values (?, ?, ?, ?)';
            ...
            if (isvalid) {
                ...
                db.exec(queryLogin, valuesLogin, conn).then(results => {
                    ...
                    const valuesUsuario =
                        [user.nmusuario, user.dsemail, user.nrcelular, results.insertId];
